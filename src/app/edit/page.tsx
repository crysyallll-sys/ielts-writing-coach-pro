...
interface EditData {
  ...
  grading_result: {
    ...
    grammar_errors: Record<string, Array<{
      original: string;
      correction: string;
      context: string;
      reason: string;
...
function EditContent() {
  ...
  if (editData.grading_result.grammar_errors) {
    const ge = editData.grading_result.grammar_errors as Record<string, GrammarError[]>;
    errorCategories.forEach((cat) => {
      const errors = ge[cat] || [];
      errors.forEach((err) => {
        grammarErrors.push(err);
      });
    });
  }
        ...
        <div className="flex flex-col lg:flex-row gap-6">
                ...
                <ul className="space-y-2 max-h-[300px] overflow-y-auto">
                  {grammarErrors.map((err, idx) => (
                    <li key={idx} className="text-red-600 bg-red-50 p-3 rounded-lg text-sm">
                      ...
                      {err.reason && <span className="text-gray-500 text-xs ml-2">({err.reason})</span>}
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <div className="flex-1 bg-white rounded-2xl shadow-xl p-6">
            ...
            <textarea
              ...
              className="w-full h-[500px] p-4 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-none text-gray-800 font-mono text-sm"
            />
          </div>
        </div>
